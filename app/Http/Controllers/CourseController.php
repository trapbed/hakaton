...
// AUTHOR
    ...
    public function rate_course(Request $request){
        ...
        $rate = DB::table('rate_course')->insert([
        ...
        if($rate){
            ...
            return redirect()->route('complete_course', ['id_course'=>$request->id_course])->withErrors(['er'=>'Вы завершили курс! Спасибо за оценку!']);
        }
        else{
            return redirect()->route('one_course_main', ['id_course'=>$request->id_course])->withErrors(['er'=>'Вы завершили курс']);
        }
    }
}
